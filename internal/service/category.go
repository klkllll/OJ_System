...
func CategoryModify(c *gin.Context) {
	...
	// 要修改的数据已经软删除了
	noCategory := new(models.CategoryBasic)
	err := models.DB.Where("identity = ?", identity).First(&noCategory).Error
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "参数不正确, 无此category",
		})
		return
	}
	category := &models.CategoryBasic{
...
func CategoryDelete(c *gin.Context) {
	...
	// 数据库中没有这个参数
	noIdentity := new(models.CategoryBasic)
	err := models.DB.Where("identity = ?", identity).First(&noIdentity).Error
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "参数错误，无此identity",
	...
	var cnt int64
	// 去ProblemCategory查有没有此Category的identity的条目，如有，而分类条目下有题目，不能删除。      下面这里是由identity查到id是多少。
