...
func GetProblemList(c *gin.Context) {
	...
	page = (page - 1) * size // 从数据库拿到数据的初始位置
	var count int64
...
func GetProblemDetail(c *gin.Context) {
	...
	err := models.DB.Where("identity = ?", identity). // 这里得预加载，不然嵌套的json是显示不出来的
...
func ProblemCreate(c *gin.Context) {
	// 这里的冒红不管，goland的问题
	in := new(define.ProblemBasic) // 这里是用define里的ProblemBasic，用于接收用户创建问题的参数数据，把参数封装了一下，就不用写那么多数据了，与models里的是不一样的
	...
	if in.Title == "" || in.Content == "" || in.OriginCode == "" || len(in.ProblemCategories) == 0 || len(in.TestCases) == 0 || in.MaxRuntime == 0 || in.MaxMem == 0 {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "参数不能为空",
		})
	...
	for _, v := range in.TestCases { //这里的TestCases是define里的，和models里的定义不一样
		// 举个例子 {"input":"1 2\n","output":"3\n"}
		testCaseBasic := &models.TestCase{
			Identity:        helper.GetUUID(),
			ProblemIdentity: identity, // 这里是用前面创建好的problemIdentity。
			Input:           v.Input,
			Output:          v.Output,
			CreatedAt:       models.MyTime(time.Now()),
			UpdatedAt:       models.MyTime(time.Now()),
	...
	// 创建问题 , 会创建好problemBasics和ProblemCategories和testCaseBasics三张表的数据
...
func ProblemModify(c *gin.Context) {
	...
	if err := models.DB.Transaction(func(tx *gorm.DB) error {
		...
		// 关联问题分类的更新（有个问题，为什么不直接修改，而是先删除再新增）
		// 1、删除已存在的关联关系
		...
		for _, id := range in.ProblemCategories { // in.ProblemCategories实际上是category_id
		...
		for _, v := range in.TestCases { // 这里的TestCases是 define.TestCases
			// 举个例子 {"input":"1 2\n","output":"3\n"}
