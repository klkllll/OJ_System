...
var (
	...
	appid     = "80e3cc46"
	apiSecret = "YzgwNzMzNzlhNTE2NzE5NTE0NmI5ZmRi"
	apiKey    = "ac6c495eaeffc3cb339025726c3c4987"
)
...
func TestXunfei(t *testing.T) {
	...
	go func() {
		...
		data := genParams1(appid, "汽车的手动档和自动档的区别是什么？")
		conn.WriteJSON(data)
	...
	data := map[string]interface{}{ // 根据实际情况修改返回的数据结构和字段名
		...
		"parameter": map[string]interface{}{ // 根据实际情况修改返回的数据结构和字段名
			"chat": map[string]interface{}{ // 根据实际情况修改返回的数据结构和字段名
				"domain":      "generalv3.5", // 根据实际情况修改返回的数据结构和字段名  我用的是v3.5的版本，所以这里得改
				"temperature": float64(0.8),  // 根据实际情况修改返回的数据结构和字段名
				"top_k":       int64(6),      // 根据实际情况修改返回的数据结构和字段名，从k个候选中随机选择⼀个（⾮等概率）
				"max_tokens":  int64(8192),   // 根据实际情况修改返回的数据结构和字段名，模型回答的tokens的最大长度
