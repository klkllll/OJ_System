<template>
  <div class="ques-list">
    <!--新增分类  -->
    <el-dialog v-model="sortDialog" :title="selectSort.id ? '编辑分类' : '新增分类'" width="30%" :before-close="closeSort">
      <el-input v-model="selectSort.name" placeholder="请输入分类名称"></el-input>
      <template #footer>
    ...
    <!-- 新增问题的弹窗 -->
    <el-dialog v-model="quesDialog" :title="selectQues.id ? '编辑问题' : '新增问题'" width="70%" :close-on-click-modal="false"
      :before-close="closeQues">
      <addQues @cancel="closeQues" :selectQues="selectQues" @submit="subQues" :sortList="sortList" v-if="quesDialog">
      </addQues>
    </el-dialog>
    <!--  -->
    <div class="flex-box">
      ...
      <div class="sort-list">
        ...
        <div v-for="item in sortList" :key="item.id" @click="getProblem(item.identity)"
      ...
      <div class="list" v-loading="loading">
        <h3>
          <el-input style="width: 200px; margin-right: 10px" v-model="keyword" clearable @change="getProblem(actSort)"
            @clear="getProblem(null)" placeholder="请搜索" :suffix-icon="Search" />
        ...
        <div class="scroll">
          <div class="item" v-for="item in quesList" :key="item.id">
            ...
            <div class="edit">
              ...
              <el-icon title="编辑" @click="editQues(item)">
                <edit />
              </el-icon>
            </div>
          </div>
...
import { ElMessageBox, ElMessage, MessageParamsTyped } from "element-plus";
...
const quesList = ref(<any>[]);// 问题列表
const sortList = ref(<any>[]);// 分类列表
const sortDialog = ref(false);// 分类的弹窗
const selectSort = ref({ name: "", id: '' });

const quesDialog = ref(false)// 问题的弹窗
const selectQues = ref(<any>{})// 选择的问题
...
const delSort = (item: any) => { // 删除分类
  ElMessageBox.confirm("确定要删除该分类吗?", "提示", {
    ...
    .then(() => {
      api.delSort({ identity: item.identity }).then((res: { data: { code: number; msg: MessageParamsTyped | undefined; }; }) => {
...
const getProblem = (sortId: number | null) => {
  ...
  api
    ...
    .then((res: { data: { data: { list: any; count: number; }; }; }) => {
      loading.value = false;
      if (res && res.data) {
...
const getSortList = () => {
  api.getSortList({}).then((res: { data: { data: { list: any; }; }; }) => {
    if (res && res.data) {
...
const subSort = () => {
  if (selectSort.value.name) {
    if (selectSort.value.id) {
      api.editSort(selectSort.value).then((res: { data: { code: number; msg: MessageParamsTyped | undefined; }; }) => {
        if (res.data.code == 200) {
          ElMessage.success(res.data.msg);
          closeSort();
          getSortList()
      ...
      api.addSort({ name: selectSort.value.name }).then((res: { data: { code: number; msg: MessageParamsTyped | undefined; }; }) => {
        if (res.data.code == 200) {
          ElMessage.success(res.data.msg);
          closeSort();
          getSortList()
